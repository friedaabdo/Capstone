<template>
  <div class="attractions">
    ...
    <section
      v-for="attraction in attractionsArray"
      :key="attraction.id"
      class="attr-row"
    >
...
export default {
  ...
  methods: {
    async loadAttr() {
      ...
      let url = `https://test.api.amadeus.com/v1/reference-data/locations/pois?latitude=${
        this.cityProps.lat
      }&longitude=${this.cityProps.long}&radius=${
        !this.radius ? 1 : this.radius
      }&page%5Blimit%5D=10&page%5Boffset%5D=0&categories=${this.categories.join()}`;
